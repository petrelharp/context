...
required.input <- c("modelfile","longwin","shortwin","leftwin","tlen")
...
total.counts <- 1e8
expected.counts <- predictcounts(opt$longwin, opt$shortwin, opt$leftwin, initcounts=rep(total.counts/nrow(genmatrix),nrow(genmatrix)), genmatrix=genmatrix, projmatrix=projmatrix, tlen=opt$tlen) 
...
likfun.grids <- likelihood.surface( model, tlen=opt$tlen )
```
