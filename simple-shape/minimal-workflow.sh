...
Rscript ../sim-seq.R -c $MODEL -t .01 -s 1000 -o $SIMFILE -m $SIMGENMAT || (echo "sim-seq failed"; exit 1)
...
Rscript ../make-genmat.R -c $MODEL -w ${LONGWIN} -o ${GENMAT} || (echo "make-genmat failed"; exit 1)
...
../templated-Rmd.sh ../simulation.Rmd $FITFILE $BASEDIR/sim.RData || (echo "templated-Rmd failed"; exit 1)
