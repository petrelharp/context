...
simseq <- function (seqlen, tlen, mutpats, mutrates, selpats=list(), selcoef=selcoef, patlen, bases=c("A","C","G","T"), count.trans=FALSE, initseq, basefreqs=rep(1/length(bases),length(bases)), ... ) {
    ...
    mutrates <- unlist(mutrates) / (patlen-mutpatlens+1)  # avoid overcounting (see above)
