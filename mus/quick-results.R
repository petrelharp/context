...
z <- lapply(fn,function(x){ load(x); c(as.matrix(estimates)['mle',],shortwin=shortwin,lrwin=leftwin) })
...
axis(1,labels=x$sp[x$shortwin==1],at=1:sum(x$shortwin==1))
...
x <- zz[[2]]
matplot( x[x$shortwin==1,1:18],type='l', xaxt='n', col=rainbow(24), lty=1 )
axis(1,labels=x$sp[x$shortwin==1],at=1:sum(x$shortwin==1))
if (any(x$shortwin==3)) { matlines( x[x$shortwin==3,1:18], col=rainbow(24), lty=2 ) }
