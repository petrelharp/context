...
adhoc <- rowMeans( sapply( adhoc, function (x) x[1,]/x[2,] ) )
...
for (k in seq_along(mutpats)) {
    ...
    scalemat[1+k,1+nmuts+tobases] <- (-1)*fac
}
...
scalemat[,1+nmuts+(1:nfreqs)] <- sweep( scalemat[,1+nmuts+(1:nfreqs)], 1, rowMeans(scalemat[,1+nmuts+(1:nfreqs)]), "-" )
