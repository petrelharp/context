...
option_list <- list(
        ...
        make_option( c("-m","--field"), type="numeric", default=0.0, help="External field (gamma)."),
...
if (interactive()) { seqlen <- 1e3; tlen <- .1; interaction <- 1; field <- .5; initfreq <- 0.5 }
...
initfreqs <- c(initfreq,1-initfreq)
...
save( thisone, now, bases, mutpats, mutrates, selpats, selcoef, fixfn, seqlen, tlen, initfreqs, simseqs, file=outfile )
