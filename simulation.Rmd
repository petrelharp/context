...
counts_table <- function(in_longwin, in_shortwin, half_n_show) {
    if ( in_longwin > longwin(model@counts) || in_shortwin > shortwin(model@counts) ) {
        ...
        resids = computeresids(model,
                ...
                counts=model@counts)
        true.resids = computeresids(true.model,
                in_longwin=in_longwin,
                in_shortwin=in_shortwin,
                in_leftwin=floor((in_longwin-in_shortwin)/2),
                counts=model@counts)
        resids$sim.expected <- true.resids$expected
...
true.model <- model
true.model@mutrates <- simseq.config$tip$mutrates
true.model@selcoef <- simseq.config$tip$selcoef
if (length(true.model@params)>0) { true.model@params <- simseq.config$tip$fixfn.params }
...
mr_compare$names <- names(coef(model))
```
...
The likelihood surface about the fitted and simulated values:
```{r, echo=FALSE, results="asis", fig.align='center'}
f <- function (x) { model@likfun(x*timevec) }
for (k in 1:nrow(mr_compare)) {
    pvals <- seq( .9*min(mr_compare[k,c("fit","simulated")]), 1.1*max(mr_compare[k,c("fit","simulated")]), length.out=20 )
...
if (class(model)=="contextMCMC") {
    ...
    for (k in unique(param_sizes)) {
        print( { 
            matplot( model@results$blen*subseq, x[,param_sizes==k], type='l', col=1:5, lty=1:6, xlab='number of MCMC iterations', ylab='parameter values' )
