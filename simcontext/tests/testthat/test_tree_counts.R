...
config <- contextual:::parse.models( treeify.config( read.config("big-tree-model.json") ) )
...
test_that("Counts agree with reordered ones.", {
    expect_equal( counts.1.2@counts, counts.2@counts )
    expect_equal( counts.1.2@colpatterns, counts.2@colpatterns )
    expect_equal( counts.2.1@counts, counts.1@counts )
    expect_equal( counts.2.1@colpatterns, counts.1@colpatterns )
})
