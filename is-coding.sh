...
do
    ...
    do
        ...
        # check:
        nnc=$( zcat ${chrom}.rawTAC.noncoding.gz | wc -l )
        noc=$( zcat ${chrom}.rawTAC.nonorfcoding.gz | wc -l )
        oc=$( zcat ${chrom}.rawTAC.orfcoding.gz | wc -l )
        echo $chrom $codon " : "$(zcat ${chrom}.rawTAC.gz | wc -l) "=" $nnc "+" $noc "+" $oc "=" $(($nnc+$noc+$oc))
    done
done
