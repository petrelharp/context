...
We can compute an approximation to this probability as in \eqref{eqn:window_approx}
by discarding those sites from $X$ and $Y$ that are approximately independent of $Y_i$.
Which are these?
...
We can then simply express our likelihood as
\[
    \like(y \mid x) = \frac{ A_{2\ell+1, \ell+1, \ell}(x,y) }{ A_{2\ell+1, \ell, \ell+1}(x,y) }.
\]
