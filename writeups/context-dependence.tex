...
we will compute the matrix of probabilities that a ``long'' sequence observed at that node
is matched by the ``short'' sequences seen on all the tips of the tree
further away from $a$ (i.e.\ below that node if it was rooted at $a$).
...
\begin{enumerate}
  ...
  \item While there are $u_j$ whose children $w_1$ and $w_2$ both have matrices computed,
    \begin{enumerate}
      \item let
        ...
        \end{align}
        where $t(w_1)$ and $t(w_2)$ are the lengths of the respective branches.
    \end{enumerate}

  \item Let 
    \begin{align}
      M(\rho)_{x,y} = \pi(x) \left( e^{t(w(\rho)) G} M(w(\rho)) \right)_{x,y}
    \end{align}
  ...
  \item For each $1 \le k < \ell$,
    \begin{enumerate}
      \item let
        \begin{align}
          M(v_k)_{x,(y,z)} = \left( e^{t(v_k) G} M(v_{k-1}) \right)_{x,y} \left( e^{t(u(v_k)) G} M(u(v_{k-1})) \right)_{x,z}  .
        \end{align}
    \end{enumerate}

  \item Finally, let
    \begin{align}
      M(a)_{x,y} = \left( e^{t(a) G} M(v_{\ell-1}) \right)_{x,y}  .
    \end{align}
