...
each of which taking one of a finite collection of possible states,
...
but this is impractical as the size of the matrix is equal to the number of possible configurations.
...
and that multinucleotide substitutions are relatively common \citep{schrider2011pervasive,terekhanova2013prevalence}.
...
\citet{siepel2004phylogenetic} introduced a method to compute likelihoods under a context-dependent mutation model
by moving iteratively along the genome,
while
\citet{lunter2004nucleotide} and \citet{hobolth2008markov} used data augmentation and an MCMC algorithm to do Bayesian inference;
both these methods are promising but very computationally intensive.

Another canonical example is the Ising model of statistical physics
