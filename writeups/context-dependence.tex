...
%EM If we think that this can be generalized to the setting of position-specific local models, then let's mention that here and then put in a little section.

To formalize this notion of local dependence,
first define a \textit{pattern} to mean a contiguous sequence of states;
%EM Do we use this length notation? I couldn't easily find an instance.
...
%EM I think we could have both, but certainly this biological interpretation.
%Then we would have "selection" and "fitness" rather than "potential" and
%"energy"?
%EM Yes. Seems nice unless you think that this "fitness" model would just annoy a pop gen person.
...
%EM I'm trying to understand why I should think of e_i as an energy *change*. Why not just an energy of pattern i? We already take differences of energies.
...
\begin{example}[Genomic GC content]
    ...
    %EM Is this table clearer than just saying "set the energy of pattern C or G to be $s$...."
    \begin{center}
        \begin{tabular}{cc}
        pattern  &  energy \\
        \nC{} or \nG   &  $s$
    ...
    Also, let
    \begin{align*}
    %EM Does it make more sense to have the N here in the selection component, or in the mutation component with \mu?
    %EM Also, could we stick with $s$ for this equation? I don't see a compelling reason to switch to e, which doesn't appear outside of the context of this equation as far as I can see.
...
\begin{example}[Gibbs sampling of the Ising model]
    ...
    %EM re i+1 in the following equation: as soon as I read this I'm already wondering what happens at the end of the sequence. Could we specify that here? Are we doing a loop?
...
\[
%EM I think we want h and not j here in the parenthetical superscript.
%EM Perhaps we want \mu^{j} rather than \mu^{(j)}? I think it could be clearer to keep the latter just for subsequences. How about lower-j?
%EM And do we want to define that j is some meta-index for all triples?
...
%EM Are we happy with leaving the \sum_{J(i,x,y)} a little vague here? I think it's "j" for which there is a triple?
...
%EM Do we need this following sentence? If so, we need to update it to also say something about the fitness term.
...
%EM Here or somewhere else do we need to say something about setting the overall rate of the process? In phylogenetics it is common to scale the rate matrix so that substitutions happen with rate 1.
...
%EM I note that here we are re-using the "possible change" terminology, which we used above for something that seems slightly different, namely the fitness setup.
%EM It seems like either we should fit these possible changes into that framework, or call these something else.
...
\[
    %EM If we change anything about j and J above, propagate that change here.
...
%EM Do we want to have sequence length become L here? It has been $n$ so far.
...
\[
q_j = \begin{cases}
    %EM Perhaps we could look at this notation together? I would think that u^{(j)} is the length-j subsequence of u, but I don't think that's what's desired here.
...
%EM I suggest that we have something like (B) in the introduction.
%EM We need to make it clear that the window width of the underlying model, and the T-mer width, are two different things.
%EM Yes, this is in the last paragraph, but I think this point gets lost when we start with "a general context-dependent model."
\begin{figure}
    \begin{center}
...
%EM Shall we define w here?
%PR No, I think that'll muddy the waters.
...
this gives us an approximation for the full likelihood as the ratio of the likelihoods of all $(2\ell+1,\ell,\ell)$ T-mers in $(x,y)$
to that of of all $(2\ell+1,\ell,\ell+1)$ T-mers.
...
%EM I know that it's all written out in the appendix, but I think that having a little more development here would be nice.
%EM Can we re-instate the material just above, that's commented out?
%EM Also, what about the edge cases? When you say "all the t-mers", I think that includes t-mers where some of the "arms" are sticking off the sequence? Or are we making the sequence into a circle?
...
%EM I think n is used here in a different sense than just above.
...
%EM This is beautifully simple but I'm not used to seeing p-values as a means of doing model selection (this is my lacking stats background showing up again).
%EM Can you point me to other examples where this is done?

%%%%%%%%%%%
\section{Computation}

%EM This needs some notation updating. I think we want to commit to the n, \ell, r notation just above? I can do this.
...
%EM Is it worth noting that (IIUC) we could have additional sparsity based on the underlying (not T-mer) model? E.g. when you are are doing stepwise addition of parts to your model, you aren't using all possible 2m+1 - mers?
...
%EM What if we want to do derivative-based optimization of t? Then we'll have sparse times exp(sparse) times U. Still seems fast?
...
%EM This is true for the implementation, but it's not clear that we couldn't do some of the same tricks for calculating likelihoods if we were handed a model.
...
%EM Do we care that N is used for different things elsewhere?
