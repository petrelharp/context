...
%EM My suggestion is "Inference using context-dependent models of mutation via bounds on interaction propagation"
...
The difficulty of context-sensitive models is that, in principle, one may have a sequence of events with cascading consequences that involves a lot of sequence.
...
However, the point of the work presented here is that for typical models such a sequence of events is improbable.
...
\begin{example}[Genomic GC content]
    ...
    then $\mu \, N \phi(E(y)-E(x))$ is the rate at which the mutation
...
\[
    %EM I think this n is supposed to be L.
    \mu^* = \max \left\{ \sum_{y \in \calS^n} \sum_{(\mu, u, v) \in J(i,x,y)} \mu \qquad \st x \in \calS^n, \quad 1 \le i \le n \right\} .
\]
...
%EM My counter-argument is that we need to make it clear that the overhang of the T-mers is different than than the context-sensitivity width.
