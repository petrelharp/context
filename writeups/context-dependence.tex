...
we'd compute the likelihood for situations where $y$ and $z$ agree (but may differ from $x$
...
The top and bottom of the table of (2,1,0) and (2,0,1) T-mer residuals of this model were
as shown in Table~\ref{tab:resid1}.
...
\begin{table}
  ...
  \end{center}
  \caption{
      True parameter values, and estimates obtained
      from iterative fitting to a $10^6$ bp sequence
...
% cpg-plus-epsilon/32140/base-model-fit-5-3-l1-resids.2.1.l1.tsv
    ...
    \caption{
        ...
        as well as $TCC \to TTC$ and $AGG \to AAG$ mutations.
...
but none so large as those seen in (3,1,1) T-mer residuals, shown in Table~\ref{tab:resid2}.
...
\begin{table}
    ...
    \caption{
        ...
        (single-nucleotide) and CpG mutations to sequence simulated under the mdoel
        of Table~\ref{tab:cpg_results}, that also includes $TCC \to TTC$ and $AGG \to AAG$ mutations.
        Note that in the table, the underscore means ``any nucleotide''.
        \label{tab:resid2}
    }
\end{table}
