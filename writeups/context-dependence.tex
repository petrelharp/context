...
% TODO include github information

%EM I'm going to suggest "dependence propagation" rather than information
...
%EM I promoted this section here because we use residuals a lot in the results.
...
%EM Why shorter? Because there are fewer of them?
These \emph{residual} counts are usually most usefully computed for T-mers shorter than those used to fit the model.
%EM Should we replace a and b with x and y here to agree with the rest of the doc?
  ...
  \begin{center}
        \begin{tabular}{c@{\quad$\to$\quad}c@{\quad at rate\quad }ll}
          \hline
            x & y & truth & MLE \\
          \hline
          %EM I'm confused why we have MLE parameters for these single NT terms because I don't see those being used in the iterative model fitting. I thought we started with a (5,1,1) model.
          \nA  &   \nT           & 0.10 & 0.0996 \\
          \nT  &   \nA           & 0.10 & 0.0982 \\
          \nC  &   \nG           & 0.15 & 0.1505 \\
          \nG  &   \nC           & 0.15 & 0.1497 \\
...
%EM Stupid question: why weren't these picked up by the 5,1,1 model?
%EM Why isn't the starting point a (1,0,0) model?
These results clearly call for addition of the CpG mutation motif.

\begin{table}
%EM This table is BIG. And it seems like our conclusions only stem from the very biggest residuals.
%EM What if we cut of anything that doesn't have an absolute Z-score bigger than 10?
...
% cpg-plus-epsilon/32140/base-model-fit-5-3-l1-resids.2.1.l1.tsv
    \begin{center}
        \begin{tabular}{ccrrrr}
                ...
                %EM It looks like this is missing an entry?
...
%EM "These patterns": Is it exactly the patterns in the true model?
%EM Would we need to have prior information about CpGs to pick those additional terms?
...
%EM what is "statistically significant" here? I searched around and found 1.96 corresponding to 0.05.
...
\begin{figure}
    \begin{center}
        %EM Should I be looking at the gray bars or black bars? What are they? I'm guessing 95% is gray, because that corresponds to good coverage.
...
%EM Kelley saw the same thing https://paperpile.com/shared/aAYSgW ...
...
%EM given the full likelihood arguments, can we really say provably correct?
...
%EM Does a gap character really make sense? It seems like this changes the interpretation of the tmer length... I'd skip.
Short deletions might be accommodated by introducing a ``gap'' letter to the alphabet,
but insertions are problematic.

%EM Full likelihood conjecture?
