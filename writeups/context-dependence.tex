...
%EM suggest "every site will have 75\% chance of changing", which is a little different than what you say because of multiple hits, right?
...
\begin{align} \label{eqn:phylo_likelihood}
    %EM I think the superscript of Y_I should be (m)
    \P\{X_{I-\ell}^{(n)}(t_X)=x \text{ and } Y_I^{(\ell)}(t_Y)=y \}
    =
...
%EM In the previous full likelihood section, we discuss choosing l, r, and n, and use those in the equations.
%EM This makes the equations look a little different. Do we want to just clarify that we are writing the equations for general l, r, n, and that we refer the reader to the previous choice to choose these quantities?
assuming for the sake of notational simplicity that we have a few positions
...
The general case is a modification of Felsenstein's ``pruning'' algorithm.

\begin{figure}
    \begin{center}
    %EM We need to put these superscripts in parens, no? Also, should we use n instead of x+2l?
...
\begin{align} \label{eqn:three_taxa_likelihood}
  \begin{split}
    ...
    &\qquad
      =
      %EM I think this should be n, ell, r, not m, ell, r
      \sum_{u \in \calS^{n}} \pi(u) p_{m,\ell,r}(t_C,u,z)
      \sum_{v \in \calS^{n}} p_{n,0,0}(t_{AB},u,v) p_{n,0,0}(t_{A},v,x) p_{m,\ell,r}(t_B,v,y)
  \end{split}
\end{align}
...
%EM I'm not really following this section. If we are only calculating likelihood for places where y and z are the same, why is phylogenetic inference useful?
        ...
        and other transversions ($\mu_{r}$):
          \begin{center}
            \begin{tabular}{c@{\quad$\to$\quad}c@{\quad at rate\quad }c|c@{\quad$\to$\quad}c@{\quad at rate\quad }c|c@{\quad$\to$\quad}c@{\quad at rate\quad }c}
                %EM I think there were some typos in this table that I fixed.
                \nT  &   \nA   &  $\mu_{AT}$ & \nA  &   \nC   &  $\mu_{r}$ & \nA  &   \nG   &  $\mu_{v}$ \\
                \nA  &   \nT   &  $\mu_{AT}$ & \nC  &   \nA   &  $\mu_{r}$ & \nG  &   \nA   &  $\mu_{v}$ \\
                \nC  &   \nG   &  $\mu_{CG}$ & \nG  &   \nT   &  $\mu_{r}$ & \nT  &   \nC   &  $\mu_{v}$ \\
                \nG  &   \nC   &  $\mu_{CG}$ & \nT  &   \nG   &  $\mu_{r}$ & \nC  &   \nT   &  $\mu_{v}$ \\
...
%EM In my compilation there is a float that interrupts this flow.
...
and that the length of the sequence does not change (no indels).
