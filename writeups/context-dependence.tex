...
%EM It feels like we go back into introduction mode here after the "In this paper" paragraph.
...
\begin{example}[TASEP]
  ...
  %EM I can't actually tell if what is developed below addresses this difficulty!
...
%EM I think we could have both, but certainly this biological interpretation. Then we would have "selection" and "fitness" rather than "potential" and "energy"?
...
%EM Can we introduce the phi notation here, and say something a little more formal? Namely that if the mutation rate is mu then the actual rate of change is mu times phi of the difference of energies?
...
\begin{example}[Genomic GC content]
    ...
    %EM This "more than one way" isn't as clear as "more than one pattern may match" used above, IMHO. Because it's the same underlying substitutions, just different patterns getting us there, right?
...
%EM My understanding is that this is also called uniformization http://dx.doi.org/10.1214/09-AOAS247
...
\[
%EM Here we're moving to lower-j.
...
%EM I wonder if we could do something like introduce J(x), which is the set of all possible transitions starting at x, so we don't have to do this "unnecessary" thing. Or perhaps it's fine as is.
...
%EM Not sure what I was on about here.
...
For instance, to simulate the TASEP, one has only to let $\mu_*=\lambda$, and at each $c_{ij}$
...
%EM I don't see this needing shortening!
...
\begin{figure}
    ...
    \caption{
        ...
        %EM This "window" terminology... should we have another quick definition here?
...
%EM The full exact likelihood? Isn't it an approximation?
...
%EM Say $a$ is the first $\ell$ bases of $x$ and $b$ is the last $r$ bases of $x$.
We refer to such patterns as ``T-mers'', as depicted in Figure~\ref{fig:Tmers}.
%EM Later we use the terminology "base of the T-mer." Let's define that here.
%EM Shall we define w here?
This quantity is found by marginalizing $p_m$:
\begin{align}
%EM Are we parameterizing in terms of n,\ell,r or n,\ell,m? We see both.
...
%EM should this be p_{n,\ell,m} below?
%EM What is k here? And do you mean \ell?
is close to $p_{\ell,m}(x,y)$, regardless of context wider than $2k+l$:
\begin{align} \label{eqn:window_approx}
%EM I think I know what you're doing here but the $i$ seems rather lonely here. Do we think of it as something for marginalization or summation or something?
%EM Also, x on the left hand side appears to be the full sequence, while on the right hand side it appears to be a subsequence. Same for y.
    p_{n,\ell,r}(x,y) \approx \P\{ X_{i+\ell}^{(n-\ell-r)}(t) = y \mid X_i^{(n)}(0) = x \}.
\end{align}

%EM I feel like it needs emphasis that this is different than saying that we assume that the rates stay the same as in the initial sequence.
...
\begin{align*}
%EM I'm not accustomed to seeing a big chunky ampersand in prob statements, rather I typically see commas. But you be you if this is how you do it.
...
%EM The following sentence may get us in trouble with the formalization police.
...
%EM Different notation for transition triples here it seems.
...
%EM I'm going to suggest a separate sentence defining the si and sij
...
%EM Can I suggest "changes to the sequence outside..."
...
\end{gather}
%EM I'm having some difficulty following... so far H has been described as states, neighborhoods, and changes. I guess it all comes back to subsequences.
...
\begin{align} \label{eqn:prob_approx}
  %EM You want _not_ contained in for the RHS here, right?
  %EM I think I need this written out in a little more detail.
...
%EM Oh, this is VERY cool.
%EM But does this random walk ever jump to the right? If not, does left-only jumping still considered a random walk?
