\documentclass[english]{Liebert_Author_revised}
...
\addbibresource{context-dependence.bib}
...
\author{
    Frederick A Matsen IV$^{2,3,4,5}$,
    Peter L Ralph$^{1,*}$\\
$^1$Institute of Ecology \& Evolution, Department of Biology, University of Oregon \\
$^2$Computational Biology Program, Fred Hutchinson Cancer Research Center, United States \\
$^3$Department of Genome Sciences, University of Washington \\
$^4$Department of Statistics, University of Washington \\
$^5$Howard Hughes Medical Institute \\
$^*$To whom correspondence should be addressed: plr@uoregon.edu
}
...
\citet{dewitt2021nonparametric} has even identified how these mutational signatures in humans
have varied over recent time.
...
co-occurring in comparisons between different cancer types \citep{alexandrov2013signatures,shiraishi2015simple} and
between different human populations \citep{harris2015evidence,harris2016rapid,mathieson2017differences}
...
while \citet{siepel2004phylogenetic} and later \citet{baele2010modelling} computed likelihoods by assuming a model that is Markov along the genome.
...
Parameter estimation for the Ising model without temporal dynamics
...
This general class of models is known in the probability literature as
...
\begin{example}[TASEP]
  The \emph{Totally Asymmetric Simple Exclusion Process}
...
\begin{example}[Genomic GC content]
    ...
    with probability approximately $(1-\exp(-2 s))/(1-\exp(-4 s N_e))$ \citep{kimura1962probability}.
    If the mutation occurs at rate $\mu$ per individual,
    the total rate it occurs at in the population is $2 \mu N_e$.
    The effect of GC-biased gene conversion can be modeled as a selective pressure in favor of G/Cs.
    ...
    \begin{align*}
        \phi(e) = 2 N_e (1-\exp(-2 e))/(1-\exp(-4 e N_e)).
    \end{align*}
...
\[
    ...
    = \max_x \sum_{y \neq x} G(L)_{x,y} .
\]
Note that we do not actually need to construct $G(L)$ to find $\mu^*$:
since $\mu^*$ must only be greater than this maximum value (not necessarily equal to it),
we can take $\mu^*$ to be $L$ multiplied by the maximum ``per-site'' rate.
...
\[
    0 < s_1 < \cdots < s_N < t .
\]
(Note that this is equivalent to placing a Poisson process of possible
changes at rate $\mu^*$ independently at each of the $L$ sites.)
...
We can, however, compute \eqref{eqn:full_likelihood} using $G(n)$ for some smaller $n$,
...
and this central, smaller subsequence is called the ``base'' of the T-mer.
The overhang of the T-mers is typically bigger than the
...
$\ell$ and $r$ should be made smaller near the left and right ends of the sequence, respectively
% on the right-hand side of the equation $\ell$ should be replaced with $\min(\ell, i-1)$,
% and $r$ should be replaced with $\min(r, L-i-\ell-m+1)$.
...
If $\lambda t = 0.75$ (in which case $1-e^{-3/4}=52\%$ of the sequence will have changed!),
...
If $\lambda t = 0.25$ we would do even better with only $\ell = r = 2$.
...
%EM This i.e. seems out of place
...
The ``projection'' we need just marginalizes over long (i.e., length $n$) patterns $c$ that match the shorter
(i.e., length $m=n - \ell - r$) pattern $b$
...
after choosing $n$, $\ell$, and $r$ as above.
For the sake of notational simplicity, assume that we have a few positions
of $X$ observed before the start and after the end of the sequence
...
\begin{align} \label{eqn:three_taxa_likelihood}
  \begin{split}
      & \P\{X_{I-\ell}^{(n)}=x \text{ and } Y_I^{(m)}=y \text{ and } Z_I^{(m)}=z \} \\
    &\qquad
      =
...
The strongest context-specific signals we find are a high CpG rate (Figure~\ref{fig:biochem_results}),
and a strong \emph{negative} rate for the UV damage motif.
A negative estimate implies that the corresponding motifs mutate \emph{less}
than expected based on other rates -- so, for instance,
a lower \nG\nA$\to$\nA\nA\ rate than expected based on the \nG$\to$\nA\ rate,
perhaps because the latter rate was estimated to be particularly high.
Notably, we find a small but significant rate of UV-associated dimer formation,
also seen by \citet{harris2015evidence}.
All rates were statistically significantly nonzero except those associated with pol $\iota$.
...
such as noisy images or particle systems modeling other physical processes.
...
\paragraph{Modeling}
Careful building of complex models in this framework can be difficult,
as in variable selection more generally  (e.g., multivariate linear modeling).
...
PLR designed the method and wrote the first drafts of the code.
PLR and FAM tested the code and wrote the paper.

\section*{Authors' disclosure}
...
\begin{proof}
    ...
    formula for the $k+1^\text{st}$ factorial moment for the Poisson distribution.
\end{proof}
