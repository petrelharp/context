...
The intuition behind this fact is straightforward, and provides a way to pick the overhang lengths $\ell$ and $r$;
a precise statement of the approximation is given in Section \ref{ss:approx_pf}.
...
Then, the sequence at a particular site may directly affect the process at the neighboring $w$ sites on either side.
...
\end{align*}
We can compute these term by discarding sites outside the window of influence:
\begin{align*}
    \P\{ Y_i = y_i \given Y_{<i} = y_{<i} \;\&\; X=x \} 
    &=
        \frac{
            \P\{ Y_i = y_i \;\&\; Y_{< i} = y_{<i} \;\&\; X=x \} 
        }{
            \P\{ Y_{<i} = y_{<i} \;\&\; X=x \} 
        } \\
    &\approx
        \frac{
            p_{n,\ell,\ell}(x_{i-n+\ell}^{(n)}, y_{i-n+2\ell}^{(n-2\ell)})
        }{
            p_{n,\ell,\ell+1}(x_{i-n+\ell}^{(n)}, y_{i-n+2\ell}^{(n-2\ell-1)})
        } .
\end{align*}
The approximation depends on $\ell$ being large enough, as discussed above, and $n \ge 2\ell+1$ --
this is because (a) to compute the likelihood in the numerator of the $i^\text{th}$ site, we need the flanking $\ell$ sites (as discussed above),
and (b) to compute the denominator we need to include all sites to the left affected by those $2\ell+1$ sites.

By rearranging the product, 
this gives us an expression for the full likelihood as the ratio of the likelihoods of all $(n,\ell,\ell)$ T-mers
and the likelihood of all $(n,\ell,\ell+1)$ T-mers.
...
The algorithm in general is written out in Appendix \peter{ref it}.
...
\peter{from simulation: parameter estimation; model selection}

\peter{do a three-taxon tree? infer with shape data?}
