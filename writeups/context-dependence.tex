...
Thus given two patterns $u$ and $v$ of common length $\ell$, saying ``pattern $u$ changes to $v$ at rate $\mu$'' means that if $(X_i(t), \ldots, X_{i+\ell}(t)) = (u_1, \ldots, u_\ell)$ then
\[
\P\{(X_i(t+dt), \ldots, X_{i+\ell}(t+dt)) = (v_1, \ldots, v_\ell) = \mu dt + o(dt),
\]
...
\]
\todo{Do you prefer 1-indexing or 0-indexing for the sequences? Seems like 1-indexing before}
...
\todo{I'm a little confused by this use of "edge effects." Are you using it to mean non-independence between blocks generally?}
