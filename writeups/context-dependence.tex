...
% Let $x_i^{(h)} = (x_i, x_{i+1}, \ldots, x_{i+h-1})$ be the subsequence of length $h$ beginning at location $i$, and
...
we do expect $p_{\ell,m,r}(x,y)$ to asymptotically correct when $\ell$ and $r$ are large:
...
Now we will now calculate the mean and covariances of $N_\ell(x,y)$.
From the definitions above,
the expected number of times $y$ would appear juxtaposed with $x$ is approximately $N(x) p_{\lyl}(x,y)$.
