...
Each example above describes a 1-dimensional grid of sites, each of which can take one of a finite set of states,
...
we assume that $\{X(t)\}_{t \ge 0}$ is a Markov process on $\calS^n$
...
To make this notion of local dependence formal,
first define a \textit{pattern} to mean a contiguous sequence of states;
let $|u|$ denote the length of the pattern $u$;
and let $x_i^{(h)} = (x_i, x_{i+1}, \ldots, x_{i+h-1})$ denote the subsequence of $x$ of length $h$ beginning at location $i$.
The dynamics are determined by the set of allowed transitions and associated rates:
...
A final note: recall we are focusing on finite sequences.
By default, we do not allow patterns to ``hang off'' the end of the sequence.
...
\begin{example}[CpG mutation]
  ...
  than either single nucleotide change.
...
and in this region $x$ matches with $u$ while $y$ matches with $v$.

As noted above, there may be more than one way to mutate a sequence $x$ to get $y$:
for each position $i$, these are those transitions in $\calT$ that can be applied at $i$
to change $x$ into $y$, denoted  $J(i,x,y) = \{ (\mu^j,u^j,v^j) \in \calT \st x \xrightarrow{i,u^j,v^j} y \}$.
...
using a variant of the ``jump chain representation'' (also known as the Gillespie algorithm \citep{vankampen}).
...
The rate $\mu_*$ should be the maximum ``local'' rate at which transitions occur, per site:
\[
\mu_* = \max \left\{ \sum_{y \in \calS^n} \sum_{j\in J(i,x,y)} \mu_j \qquad \st x \in \calS^n, \quad 1 \le i \le n \right\} .
\]
Note that since mutation is homogeneous, the maximum rate of changes occuring at any position will be the same, except possibly near the end of the sequence,
so the maximization over $i$ is unnecessary.
\todo[inline]{OK, I'm getting confused now. From the above, I would have thought that $\mu_*$ was the overall rate of mutation, but now it's only happening from pattern x?}
\peter{$\mu_*$ is the maximum transition rate per site.  better now?}
...
\]
is a rate $\mu_*$ Poisson process on $[0,t]$, and these are independent across sites.
\todo[inline]{Further confusion: now $\mu_*$ is just the rate at a single site?}
\peter{yes.}
Now, suppose we have determined the state at time $s$ to be $X(s) = x$,
and the next event is $c_{ij} = \min_{k,h} \{ c_{kh} : c_{kh}>s \}$.
At $c_{ij}$,
a new subsequence is then chosen at position $i$
with probability proportional to the corresponding rate:
\[
q_j = \begin{cases}
  \mu^j/\mu_* \qquad & \text{if } x_i^{(|u^j|)} = u^j  \\
  0 \qquad & \text{otherwise,}
\end{cases}
\]
is the probability that $x_i^{|u^j|}$ is replaced with $v^j$,
and no chance is made with the remaining probability $q_0 = 1-\sum_{j=1}^{n_T} q_j$.
...
\todo[inline]{I would think that this is identifiable from the stationary distribution if we know the structure of the model.}
\peter{What is identifiable?  The entire model, up to a time change?  I go back and forth on this...}
...
is close to $p_{\ell,m}(x,y)$, regardless of context:
\begin{align} \label{eqn:window_approx}
  p_{\ell,m}(x,y) \approx \P\{ X_i^{(m)}(t) = y \mid X_i^{(2\ell+m)}(0) = x \}.
\end{align}
...
Equipped with marginal transition probabilities from length-$m+2\ell$ patterns to length-$m$ patterns
that correctly account for local dependencies,
we can now consider inference procedures.


Now we know how to compute the approximate (marginal) probability that a particular subsequence changes into another subsequence.
Let $N(x)$ be the number of occurrences of a pattern $x$ in $X(0)$.
For patterns $x$ and $y$, with both fringe widths equal to $\ell$ (so that $|x|=|y|+2\ell$),
let $N_\ell(x,y)$ count these events, respectively be the number of times $x$ appears in $X(0)$,
and how many times $x$ appears in $X(0)$ while $y$ appears at the same place in $X(t)$, offset by $\ell$.
For instance, $N_3(\text{ACTCAC}, \text{CGC})$ counts the number of occurrences of
\begin{center}
...
\newcommand{\lyl}{\ell,|y|,\ell}

Now we will now calculate the expectation and covariances of $N_\ell(x,y)$.
From the definitions above,
\[
\E [N_\ell(x,y)] \approx N(x) \, p_{\lyl}(x,y).
\]
To compute covariances, define $M_i(x,y)$ to be 1 if $X_{i-\ell}^{(|x|)}(0)=x$ and $X_i^{(|y|)}(t)=y$, and 0 otherwise,
so that $N(x,y) = \sum_{i=1}^n M_i(x,y)$.
\todo[inline]{
In the below, are you are using an equivalence
\[
\cov[M_1(x,y),M_{k+1}(x,y)] = \E[M_1(x,y) M_{k+1}(x,y)] - p_{\lyl}(x,y)^2
\]
How do you justify this? I don't think that $M_1(x,y)$ is equal to $p_{\lyl}(x,y)$: the former is defined in terms of observed events, whereas the latter is defined starting with "if we have a sequence $x$" without any reference to the frequency of $x$.
For example, what if $x$ doesn't appear in $X(0)$?
I was with you on $\E [N_\ell(x,y)] \approx N(x) \, p_{\lyl}(x,y)$.
}
