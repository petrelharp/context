...
It will often be convenient in the notation to avoid edge effects
by treating the $n$ sites as a circle, and treating all indices modulo $n$.
\todo[inline]{So shall we commit to this? I think that would be best, and then explain any deviation from it individually.}
\peter{I think it would be better to remove this, but I need to parse the remainder...}
...
\todo{OK, rad.}
The rate $\mu_*$ should be the maximum ``local'' rate at which transitions occur at any position:
\[
...
\peter{$\mu_*$ is the maximum transition rate out of any possible pattern $x$.  I think the notation is correct? I have tried to clarify.}
Note that since mutation is homogeneous, the maximum rate of changes occuring at any position will be the same, except possibly near the ends of the sequence.
...
\]
is a rate $\mu_*$ Poisson process, and these are independent across sites.
...
and the next event is $s_{ij} = \min_{k,h} \{ s_{kh} : s_{kh}>s \}$.
The process then jumps to a new state with probability proportional to the corresponding rate.
In other words, let
\[
q_k = \begin{cases}
...
we will lose most of the information about the dynamics,
...
\]
\todo[inline]{Do you prefer 1-indexing or 0-indexing for the sequences? Seems like 1-indexing before}
Variations are possible, such as modifying edge effects behavior for $p_m$.

However, in any case there is no guarantee that this is close to correct,
...
\todo[inline]{I'm a little confused by this use of "edge effects." Are you using it to mean non-independence between blocks generally?}
...
We will call these left and right \emph{fringes}.
...
\todo[inline]{Is the following notation right? It seems like taking it out is a viable option: "In a long sequence, $p_m(x,z)$ might not be a very good approximation for $\P\{ X_i^{(m)}(t) = z \mid X_i^{(m)}(0) = x \}$,"}
We expect $p_{\ell,m,r}(x,y)$ to be asymptotically correct when $\ell$ and $r$ are large \todo{and $t$ is not} :
\begin{align} \label{eqn:window_approx}
  p_{\ell,m,r}(x,y) \approx \P\{ X_i^{(m)}(t) = y \mid X_i^{(\ell+m+r)}(0) = x \}.
\end{align}
We provide a precise statement of this approximation later.
