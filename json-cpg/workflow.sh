...
echo "fit a model"
Rscript ../fit-model.R -i ${BASEDIR}$BASE-123456-${LONGWIN}-root-${SHORTWIN}-tip-l${LEFTWIN}.counts -m $GENMAT -j 54321
...
Rscript ../fit-model.R -i $BASE-123456.4.2.l1.counts -m genmatrix-4-cpg-model.RData -j 54321

Rscript ../count-seq.R -i $BASE-123456.RData -w 5 -s 3 -l 1
Rscript ../make-genmat.R -c ${MODEL} -w 5
Rscript ../fit-model.R -i $BASE-123456.5.3.l1.counts -m genmatrix-5-cpg-model.RData -j 54321

Rscript ../count-seq.R -i $BASE-123456.RData -w 6 -s 2 -l 2
Rscript ../make-genmat.R -c ${MODEL} -w 6
Rscript ../fit-model.R -i $BASE-123456.6.2.l2.counts -m genmatrix-6-cpg-model.RData -j 54321
