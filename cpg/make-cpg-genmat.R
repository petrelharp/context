...
fixfn <- function (...) { 1 }
if (meanboundary>0) {
    genmatrix <- meangenmatrix( leftwin=meanboundary, rightwin=meanboundary, patlen=longwin, mutpats=mutpats, selpats=list(), boundary=boundary )
} else {
    genmatrix <- makegenmatrix( patlen=longwin, mutpats=mutpats, selpats=list(), boundary=boundary )
}
