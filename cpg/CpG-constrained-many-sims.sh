...
MODELNAME="$(echo $MODEL|sed -e 's/.json//')"
...
do
    (
        ...
        echo "Simulation $N, in $SUBDIR/sim-$DIR";
        mkdir -p "$SUBDIR/sim-$DIR";
        # simulate up some sequence for testing;
        Rscript ../scripts/sim-seq.R -c $MODEL -t $TLEN -s 100000 -d "$SUBDIR/sim-$DIR" -o "sim.RData";
        for LONGWIN in 3 4 5 6;
        do
            SHORTWIN=$(( LONGWIN/2 ));
            ...
            # and count the Tmers;
            Rscript ../scripts/count-seq.R -i $SUBDIR/sim-$DIR/sim.RData -w $LONGWIN -s $SHORTWIN -l $LEFTWIN;
            # fit the model;
            FITFILE="$SUBDIR/sim-$DIR/test-cpg-fit-${LONGWIN}-${SHORTWIN}-l${LEFTWIN}.RData";
            Rscript ../scripts/fit-model.R -c $MODEL -t $TLEN -i "$SUBDIR/sim-$DIR/sim-${LONGWIN}-root-${SHORTWIN}-tip-l${LEFTWIN}-shift0.counts" -m "genmatrices/genmatrix-${LONGWIN}-${MODELNAME}.RData" -o $FITFILE;
            Rscript ../scripts/gather-results.R --fit $FITFILE --sim $SUBDIR/sim-${DIR}/sim.RData --outfile $SUBDIR/sim-$DIR/fit-${LONGWIN}-${SHORTWIN}-l${LEFTWIN}.json --json 2>/dev/null ;
        done;
    ) &
done
