...
mkdir -p $GMDIR
for THISMODEL in $TRUE_MODEL $BASEMODEL
do
...
DIR=$BASEDIR/32140
        ...
        do
            ...
            Rscript ../scripts/fit-model.R -c $BASEMODEL -t $TLEN -i "$DIR/sim-${LONGWIN}-root-${SHORTWIN}-tip-l${LEFTWIN}-shift0.counts" -m "$GMDIR/genmatrix-${LONGWIN}-${MODELNAME}.RData" -o $FITFILE;
