...
showresids () {
    ...
    (for x in RegulatoryFeature-regions-from-axt/**/$MODEL*resids.${LONGWIN}.${SHORTWIN}.l${LEFTWIN}.tsv; do y=$(basename $x | sed -e 's/-resids.*//'); z=$(head -n $((NPATS+1)) $x | tail -n +2); echo $y $z; done) | column -t
    (for x in RegulatoryFeature-regions-from-axt/**/$MODEL*resids.${LONGWIN}.${SHORTWIN}.l${LEFTWIN}.tsv; do y=$(basename $x | sed -e 's/-resids.*//'); z=$(tail -n $NPATS $x); echo $y $z; done) | column -t
}
