...
if [ $# -lt 4 ]
then
    echo "Run *constrained* MCMC beginning from previous stopping point.\n Usage: $0 (modelfile) (longwin) (shortwin) (leftwin)"
    exit 0
fi
...
for d in $DIRS; do for t in $TYPES; do 
    ...
    then
        ( echo "MCMC on $FITFILE !";
          ...
          ../scripts/mcmc-tree-model.R -i $FITFILE -c $THIS_MODELFILE -b $NBATCH -j $JOBID -o $OUTFILE --nonnegative
